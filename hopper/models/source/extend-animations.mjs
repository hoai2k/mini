...
const OUT=path.join(ROOT,'local/hopper-animation-extension');fs.mkdirSync(OUT,{recursive:true});
...
const data={};for(const kind of ['hopper','rider','hopper-rider']){const a=read(path.join(ROOT,'hopper/models/'+kind+'.glb'));data[kind]={a,r:rig(a)}}
...
function wings(p,amount,u=0){for(const [s,sign]of [['L',1],['R',-1]]){ // Wing axes are world +Y: fan back-swept panels outward, with a small camber lift.
 H.turn(p,'wing.'+s,[0,1,0],-sign*1.03*amount);H.turn(p,'wing.'+s,[0,0,1],sign*(.14*amount+.018*Math.sin(TAU*u)*amount));}}
...
function riderPose(name,u){const p=structuredClone(sitR),e=pulse(u,0,.22,.65,1);let lean=0,look=0;
 ...
 // Deform arm chains are root siblings, so give them the same shoulder lean rotation.
 ...
 for(const [side,s]of [['L',1],['R',-1]]){
  ...
  if(name==='Point_Forward'&&side==='R'){R.turn(p,'DEF-upper_arm.'+side,[1,0,0],-.90*e);R.turn(p,'DEF-forearm.'+side,[1,0,0],.30*e)}
...
for(const [kind,{a,r}]of Object.entries(data)){const original=read(path.join(ROOT,'hopper/models/'+kind+'.glb')),startCount=a.j.animations.length;
 ...
 if(kind==='hopper-rider'){
  ...
  else channels(H,times.map(()=>structuredClone(zeroH)),hfilter);
 }
