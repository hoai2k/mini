...
text=source.read_text().replace('long four-limbed body','long body with four arms and two legs')
...
h=sec.header.paragraphs[0];h.text='HOPPER THE GRASSHOPPER     /     GAME DESIGN'
...
def table(rows):
    ...
    n=len(rows[0]);widths=([2.15,2.4,2.59] if n==3 else [1.7,1.65,1.7,2.09] if n==4 else [2.0,5.14])
    ...
    for i,row in enumerate(rows):
        ...
        for j,(cell,s) in enumerate(zip(cells,row)):
            ...
            r=p.add_run(s);r.font.size=Pt(10);r.bold=i==0;r.font.color.rgb=RGBColor.from_string('FFFFFF' if i==0 else '111111')
...
for pageidx,page in enumerate(pages):
    ...
    while i<len(lines):
        ...
        if m:
            ...
            width=7.14
            if 'canonical-v1' in path.name:width=6.4
            if 'logo' in path.name:width=6.4
            if 'icon-master' in path.name:width=0.8
            ...
            if re.match(r'^\d+\. ',line):p.paragraph_format.space_after=Pt(11)
        i+=1

doc.core_properties.title='Hopper the Grasshopper Game Design'
doc.core_properties.subject='Illustrated gameplay and art direction proposal'
doc.core_properties.author=''
doc.core_properties.keywords='Hopper, game design, Xbox, anime, concept art'
for tree in [doc.styles.element,doc.element]:
    for border in list(tree.iter(qn('w:pBdr'))):border.getparent().remove(border)
out=ROOT/'Hopper_Game_Design.docx';doc.save(out)
