"""Label the Phase Skate's existing QA renders; no source artwork edits."""
from pathlib import Path
from PIL import Image, ImageDraw, ImageFont
ROOT=Path(__file__).resolve().parents[4];OUT=ROOT/'local/hopper-phase-skate-refine'
font=ImageFont.truetype('/System/Library/Fonts/Supplemental/Arial.ttf',23)
for category,items in [('views',['front','top','rear','side']),('clips',['Glide','Fade_Out','Silhouette_Hold','Fade_In','Dash','Hit','Dissolve'])]:
 sheet=Image.new('RGB',(1100,len(items)*440),(31,38,49));draw=ImageDraw.Draw(sheet)
 for row,name in enumerate(items):
  for lod in range(2):
   file=OUT/(f'LOD{lod}-{name}.png' if category=='views' else f'LOD{lod}-front-{name}.png')
