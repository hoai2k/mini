"""Measure evaluated sole heights and render burrowing details from saved candidate."""
import bpy,sys,json
from pathlib import Path
HERE=Path(__file__).resolve().parent;sys.path.insert(0,str(HERE))
import basalt_burrower as ct
import window_ray_refine as preview
OUT=HERE.parents[3]/'local/hopper-basalt-burrower-refine'
...
for lod in range(2):
 for clip,phase in [('Tunnel',.25),('Tunnel',.75),('Erupt_Tell',1),('Land',.5),('Withdraw',.5)]:
  ...
  else:preview.render(scene,cam,roots,arms,saved,lod,'side',clip,phase)
