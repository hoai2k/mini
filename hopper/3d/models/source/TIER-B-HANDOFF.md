...
- `hopper/game/src/game3d/world.ts:101–143` builds solid collision boxes from
  stand-in mesh bounding boxes. Line **127** excludes `RibN`, but not `Spine`.
...
generator rather than the PNG. `game/src/game3d/world.ts:165` confirms
`makeHeightField(...)`; `scene.ts:302` independently generates visible terrain.
...
The three existing files span **4800×4800 m**, matching district.ts:275/449/615;
...
collision. Complete Fields first, then City/Mountains; the other six regions need
route/plateau specifications and are not delivered by reusing these three maps.
