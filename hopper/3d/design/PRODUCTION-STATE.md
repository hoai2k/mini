...
- Root accepted M-005 Window Ray appearance and all four animation review sheets for both LODs. Astra `animation_resume` is integrating it, then refining M-007 Crag Tortoise. M-011 Chain Manta scaffold is ready; Sol `creatures_resume` is completing the final M-015 Basalt Burrower scaffold. Scaffolds are not delivered assets.
- Root accepted both LOD comparison sheets for city and mountains terrain; integration pending shared-manifest availability.
- M-067 candidate visual review passed. Its original 124 LOD1 triangles include two coincident duplicate ring faces (triangles 1/0 and 41/40); Blender import removes them, yielding 122. This is an explicitly accepted cleanup, not a stale manifest count. Sol is adding source/import topology guards and the narrowly scoped exception before integration.
- Remaining mechanical holds: M-056 original landing lacks a nearby face; M-059/060/064/066 need a functional-hierarchy-preserving path. Tier B is not marked complete. Preserve both named stashes and unrelated `updates/`.
