...
As built: the jump is a spring. A held on the ground roots Hopper where he stands and winds it up; letting go launches it, and the fall pulls harder (1.4×) so the arc is quick up and sharp down. A quick tap barely winds and is a small hop taken without a pause; a second and a half is the full charge. With the stick forward the arc is measured against the run it interrupted — over the wind-up and the flight together a tap keeps exactly the running pace and a full wind doubles it, so springing across a district beats running it — which at 68 m/s gives a 13 m apex over 62 m of ground for a tap, 100 m over 317 m for half a wind and 189 m over 641 m for a full one, nearly a kilometre at a sprint. Stick neutral puts the whole wind into height instead: straight up, 23 m from a tap to 345 m from a full wind, steerable all the way down. The reach comes from the arc's own flight time, so the bargain holds under every region's gravity. Holding A in the air is still the hover and then the glide. Every number that shapes the jump lives in `hopper/game/src/game3d/jumptuning.ts`, which the controller spreads into its movement constants, so the feel is retuned in one file rather than in the simulation. The stick's lower band (to 55 % of its throw) walks at up to 34 m/s and its upper band gallops to 68 m/s, twice the walk, with the sprint on top.
