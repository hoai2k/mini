...
| A | Jump: it fires on the press and holding it jumps higher, letting go on the way up cutting the rise short; keep holding past the top to hover on beating wings, and on to glide; at a wall, wall kick | Space |
...
| LB | Spring: the super jump. Held on the ground it winds up and the stick aims it; a tap under 0.3 s costs no stride | E |
| RB | Sprint (hold) | Shift |
| Left-stick click | Dash, the way the stick points, on the ground or in the air | R |
...
The mapping keeps the 2D game's muscle memory where it still means the same thing (A jump and hover, X and B kick, RT lasers, Menu, View), puts the super jump on a shoulder so the jumping thumb never leaves A, and gives the unused inputs to the new 3D verbs. Nothing essential is on a combination.
...
| Stomp bounce | 28 m (2 H); 42 m with A held, and pulled 1.9x harder on the way up | A kick downward, not a trampoline |
...
| Climb | Jump into a face taller than he is and wide enough to stand on, stick pushed at it: 27 m/s up, 19 m/s along it, no fall while he holds | A wall is a way up, but only one he jumped at; A kicks off it |
| Ledge mantle | Reach 6 m over the lip, 0.7 s | Forgives a short leap, and tops out a climb |
| Step over | Anything under 5 m is not an obstacle: no wall, no block on a jump | Kerbs and rubble never interrupt a run |
...
As built there are two jumps. **A is the ordinary one**: it fires on the press, holding it jumps higher, and letting go on the way up cuts the rise short -- a tap reaches 26 m, a full hold 96 m, and everything between is a matter of how long the button is held. The fall pulls harder (1.4×) so the arc is quick up and sharp down. **LB is the spring**, the super jump: held on the ground it roots Hopper where he stands and winds it up; letting go launches it. A quick tap of LB barely winds and costs no stride; a second and a half is the full charge. The wind sets one launch speed and the stick sets the angle it leaves at: fully forward is 15° above the ground, a flat dart that reads as a lunge rather than a jump; neutral is 45°; fully back is straight up, and a half push lands halfway between. Pulling back never throws him backwards — the backward part of the stick only stands the launch up, and whatever forward or sideways part is left gives the direction. A tap cannot reach the flattest angle (25° at no wind), so the lunge is earned, and the spring never leaves slower along its own line than he was already travelling, so a tap at a sprint keeps the sprint. At 68 m/s that gives a 4 m apex over 40 m for a forward tap, 15 m over 192 m at three quarters, 16 m over 296 m at full wind and 325 m/s, 120 m over 561 m held neutral, and a 240 m climb pulled fully back, which is the highest jump in the game. The crouch reads the whole thing in advance: he sinks deeper as the spring winds and tips back toward wherever it points, and at full wind he throbs against the stop inside a ring of light while the pad knocks once. Holding A past the top of a jump is still the hover and then the glide. Every number that shapes either jump lives in `hopper/game/src/game3d/jumptuning.ts`, which the controller spreads into its movement constants, so the feel is retuned in one file rather than in the simulation.
...
A wall can also simply be climbed, but taking hold of one is deliberately narrow, because a climb that starts by itself is a climb that interrupts running. It needs all four of: his feet off the ground, so it is something he jumped at; the stick pushed into the face; a face that carries on above his own standing height, so it is a wall rather than something he could clear; and a face at least 5 m of half-width, so there is somewhere to stand on it — a mast or a trunk is not a wall. Running at a wall now stops him at it, as anything solid does. Once he is on it gravity stops, the stick's push into the wall is the climb and its slide along the wall the traverse, and letting go of the stick leaves him hanging and sliding slowly rather than falling. A is the wall kick off it, pulling hard away from it or pressing Y lets go, and a lip within mantle reach is hauled over as it always was. Hopper lies against the face with his back to the camera while he is on it.

Below the wall is the step. Anything under 5 m — a kerb, rubble, the lip of a terrace, a low wall — is not an obstacle in any sense: it is not a face to resolve against, so it neither stops a run nor blocks a spring launched flat across it, and the ground simply lifts his feet onto its top as he crosses it. A 14 m grasshopper should not be stopped by something at his knee, and he should be able to assume a flat lunge clears it.
...
Districts are walked through across a **threshold**: a lit arch where the trail ends, with the next district visible through it in its own colours. The district ahead is not a painting: its terrain and its structures are built and placed so that its start sits exactly on the current exit, so the city grows on the horizon from the first chapter, fills the view as the last stronghold falls, and is the same city that stands there after the crossing. Only the final district of a mission shows a silhouette landmark instead.

Because the two line up exactly, crossing needs no hand-over at all, and does not get one. There is no pause, no held frame and no dissolve: the world is rebuilt under Hopper inside the step he crosses on, and he keeps the offset he crossed at, so the ground beneath his feet does not move. He arrives with the heading, the speed and the flight he crossed on, measured from the trail so that forward stays forward. A region change reads as walking on because that is all it is.

Crossing rebuilds the district's world and its shadows inside one step, so that rebuild has to be cheap enough to take in one step. It is: 60-110 ms for eight of the nine districts, and about 350 ms for Thunderhead Range, whose delivered collision bakes into 56,572 boxes. A brief hitch, not a transition.

The only thing that could interrupt it is art that has not arrived, so it is fetched before it is wanted: from 700 m out — ten seconds at a run, three at a spring — the approach loads the next district's paintings in the background, costing the run nothing either way. A threshold somehow reached cold still holds the picture until they land, rather than dropping him into a half-painted district, but in play that is not seen. Each district remains its own world with its own checkpoints, shadows and save slot; the commander still bars the last threshold of a mission until it is down.
...
Every flyer is a step. Stomping one damages it and kicks Hopper upward -- a short, sharp rebound rather than a trampoline: it reaches about two of his own heights, and the gravity on the way up is nearly double, so he leaves fast and is over the top quickly. Holding A on the bounce goes half again as high. What says the stomp landed is the snap of it, not the height. Kicks work in the air and preserve the arc. Lasers fire while gliding, so a glide toward a roof can clear the sniper on it before arrival. Pushing the stick back against the flight brakes it, which is a dodge as much as a steering correction. A chain of bounce, kick, bounce can keep Hopper aloft through a whole flock, and the flocks over the ravines and reefs are built to invite it.
