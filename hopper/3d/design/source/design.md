...
As built: the jump is a spring. A held on the ground roots Hopper where he stands and winds it up; letting go launches it, and the fall pulls harder (1.4×) so the arc is quick up and sharp down. A quick tap barely winds and is a small hop taken without a pause; a second and a half is the full charge. The wind sets one launch speed and the stick sets the angle it leaves at: fully forward is 15° above the ground, a flat dart that reads as a lunge rather than a jump; neutral is 45°; fully back is straight up, and a half push lands halfway between. Pulling back never throws him backwards — the backward part of the stick only stands the launch up, and whatever forward or sideways part is left gives the direction. A tap cannot reach the flattest angle (25° at no wind), so the lunge is earned, and the spring never leaves slower along its own line than he was already travelling, so a tap at a sprint keeps the sprint. At 68 m/s that gives a 4 m apex over 40 m for a forward tap, 15 m over 192 m at three quarters, 16 m over 296 m at full wind and 325 m/s, 120 m over 561 m held neutral, and a 240 m climb pulled fully back, which is the highest jump in the game. The crouch reads the whole thing in advance: he sinks deeper as the spring winds and tips back toward wherever it points, and at full wind he throbs against the stop inside a ring of light while the pad knocks once. Holding A in the air is still the hover and then the glide. Every number that shapes the jump lives in `hopper/game/src/game3d/jumptuning.ts`, which the controller spreads into its movement constants, so the feel is retuned in one file rather than in the simulation.
