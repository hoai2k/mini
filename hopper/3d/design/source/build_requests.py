...
model('M-000', 'Hopper and rider (combined GLB)', 'hopper', 'skeletal', [23, 19, 29], '94,925 delivered', 'hopper.proxy', 'models/hopper-rider.glb', 'delivered',
      ...
      joints='thorax, abdomen, head, 3 antenna joints per side, 3 joints per leg × 6, wing.L/R, saddle', clips='46 delivered (35 Hopper actions, 5 original rider previews, 6 new rider reactions; see hopper/models/manifest.json)', sockets='20 Hopper + 8 rider sockets (see models/manifest.json)', textures='delivered 4K colour + normal')
model('M-001', 'Hopper: traversal and air-combat clips', 'hopper', 'skeletal', [23, 19, 29], 'existing mesh', 'hopper.proxy', 'models/hopper-rider.glb (new clips)', 'delivered',
      ...
      sockets='existing plus Hopper.Wing.L/R non-deforming tip nodes for glide trails; no new skin joints', textures='existing', notes='2026-09-12: validated animation-only append to canonical hopper/models exports; original mesh, skin, textures and clips preserved. Three key poses per clip reviewed. Gameplay transitions remain to be tuned; see hopper/models/animation-extension-validation.json.')
model('M-002', 'Rider: reaction clips', 'rider', 'skeletal', [2.6, 5.6, 1.4], 'existing mesh', 'hopper.proxy', 'models/hopper-rider.glb (new rider clips)', 'delivered',
      ...
      clips='Glide_Lean (1.6 s loop), Dive_Tuck (1.2 s loop), Stomp_Brace (0.6 s), Point_Forward (1.3 s), Look_Up_Long (2.4 s), Cheer_Short (1.1 s); separate rider total 19', sockets='existing', textures='existing', notes='2026-09-12: all six reactions remain seated with zero pelvis local drift. Combined reaction clips contain no Hopper tracks. No independent scarf joint exists; cloth streaming was not added. Reviewed 18 seated key poses; gameplay transition tuning remains. See hopper/models/README.md for normal-action masking and crossfades.')
...
DELIVERED_ENEMIES = {
    'basaltBurrower': '2026-09-13: reference-authored fractured basalt quadruped delivered at 7.7 × 4.1 × 11.4 m, 6,740/1,988 triangles. Both LODs have sixteen-joint skins, three sockets and eight clips. Ringed drill, irregular dorsal shards, protected ivory core and four clawed legs reviewed through both-LOD deformation and fixed-ground support sheets. Dense 61-phase support samples pass; exported drill rotations preserve one Buried_Idle turn and two Tunnel turns. Erupt root motion is [0, 4.8, 0] m. Underground placement/root-motion application, crack trails, eruption debris, back vulnerability timing, Core/body hitboxes and dissolve shaders remain runtime responsibilities. See models/source/basalt-burrower-refine-validation.json.',
...
for cat, title, intro in [
    ('sky', 'Painted skies (9)', 'One equirectangular dome per region, drawn from inside a 9 km sphere. The stand-in is the procedural gradient-plus-sun in `paintSky`. Every delivered sky must carry native detail at its stated size - `source/verify_native_detail.py` measures it - because the dome magnifies the painting several times over at 4K.'),
