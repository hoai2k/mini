...
The live instructions diagram at `../controller-diagram.png` and its editable reference `../controller.svg` have been replaced with the updated controls. The other images are delivered assets; this change does **not** wire new animation states, props, or effects into the renderer.
...
The nine corridor plates intentionally include a dark opaque recess to make the underpass readable. Other new sprites/effects have real PNG alpha; the controller uses an opaque ivory background. Gate/corridor artwork is decoration: continue using authored collision surfaces, not painted outlines. Cage artwork includes a sentry and signal, so avoid drawing duplicate occupants beneath it.
