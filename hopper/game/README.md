...
`src/game/engine.ts` owns the fixed 120 Hz simulation, camera, parry, knockback, ledge catch, saves and combat integration. `levels.ts` defines the routes, `combat.ts` defines enemies/bosses, `renderer.ts` composites the artwork and effects, `hopper-animation.ts` selects registered cels and eye origins, and `input.ts` / `audio.ts` handle devices and sound. React owns the title, HUD, accessible menus and settings.

Run `pnpm test`, `pnpm typecheck`, `pnpm lint`, and `pnpm build`. The actual-engine traversal audit tests every one of the 405 required route transitions, including real head/side collisions. It also checks inversion recovery and laser targeting for every enemy/boss. Separate controller/audio/combat tests cover input quarantine, connection changes, looping/track routing, armor, stomps, boss phases, rear ambushes, parry staggers and reflected shots. The camera/parry/ledge-catch, level-design and animation reports are under `qa/`.
