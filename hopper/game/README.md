...
Music is provided by the user: **Hopper the Grasshopper** accompanies title/menu screens; **Grass March 1** loops through episodes one and two, and **Dark Moon** through the alien world of episode three. Track positions are preserved when changing between menus, episodes and play, and when the tab is hidden or unfocused: leaving the tab pauses the music and suspends the effect audio, and returning resumes the same track at the same position. Music and effects have separate volume controls, and a sound button sits with the other icon buttons on the play-select screen and in the HUD. The theme starts on its own where the browser permits autoplay; where it is refused the sound button shows as off until it is pressed.
