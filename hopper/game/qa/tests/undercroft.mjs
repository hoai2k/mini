...
for (let m = 0; m < 3; m++) {
  ...
  const floors = l.platforms.filter(
    (p) => p.routeRole === 'floor' && p.kind !== 'launch',
  );
  ...
  for (const u of l.undercroft) {
    ...
    for (const p of inArea)
      if (floorAt(l, p.x + p.w / 2) - p.y < UNDERCROFT.drop - 1) below = false;
    ...
    if (floorAt(l, last.x) - last.y <= floorAt(l, first.x) - first.y)
      growing = false;
  }
  check(
    below,
    `mission ${m}: the floor is at least ${UNDERCROFT.drop} under every route shelf`,
  );
  check(
    growing,
    `mission ${m}: the floor drifts further from the route along each area`,
  );
  check(
    pads.every((p) => p.launchTo !== undefined && p.launchTo < p.y),
    `mission ${m}: every pad launches upward`,
  );
  check(
    pads.every((p) =>
      mains.some(
    ...
    `mission ${m}: every pad stands under the shelf it launches to`,
  );
  const gaps = pads
    .slice(1)
    .map((p, i) => p.x - pads[i].x)
    .filter((g) => g > 0 && g < 4000);
  check(
    gaps.every((g) => g >= 300 && g <= UNDERCROFT.spacing * 2),
    `mission ${m}: pads are evenly spaced (${Math.min(...gaps)}..${Math.max(...gaps)})`,
  );
}
...
for (let i = 0; i < mains.length - 1; i++) {
  ...
  const caught = level.platforms.some(
    (p) =>
      p.routeRole !== 'floor' &&
      p.x < mid + 70 &&
      p.x + p.w > mid - 70 &&
...
engine.level = {
  ...
  hazards: [],
  collectibles: [],
  checkpoints: [{ x: a.x + 100, y: a.y, area: 0 }],
};
...
const landed = run(
  6,
  input,
  () => engine.player.grounded && String(engine.stood).startsWith('floor-'),
);
check(
  landed,
  `he lands on the dark floor (stood ${engine.stood}, y ${engine.player.y.toFixed(0)}, floor ${floorAt(level, engine.player.x)?.toFixed(0)})`,
);
...
check(
  engine.player.x < landingX - 60 && engine.player.grounded,
  `the floor carries him back (${(engine.player.x - landingX).toFixed(0)}), no jump off it`,
);
const onPad = run(
  12,
  input,
  () =>
    ...
    String(engine.stood).startsWith('launch-') &&
    Math.abs(engine.player.vx) < 1,
);
...
check(
  pad &&
    Math.abs(engine.player.vx) < 5 &&
    engine.player.x >= pad.x &&
    engine.player.x <= pad.x + pad.w,
  'the pad holds him still against the stick',
);
// The pad's jump: straight up, past the shelf over it, landing on it.
const target = level.platforms.find(
  (p) =>
    p.routeRole === 'main' &&
    p.y === pad.launchTo &&
    pad.x + pad.w / 2 >= p.x &&
    pad.x + pad.w / 2 <= p.x + p.w,
);
check(!!target, 'the pad has a shelf over it');
const x0 = engine.player.x;
engine.step(dt, { ...input, jumpPressed: true, jumpHeld: true });
check(
  engine.player.vy < -1600,
  `the launch is far past a jump (${engine.player.vy.toFixed(0)})`,
);
let apex = Infinity;
const back =
  target &&
  run(8, { ...input, moveX: 0, jumpHeld: false }, () => {
    apex = Math.min(apex, engine.player.y);
    return engine.player.grounded && engine.stood === target.id;
  });
check(
  back,
  `he lands on the shelf over the pad (stood ${engine.stood}, apex ${apex.toFixed(0)} vs shelf ${target?.y})`,
);
check(
  target && apex < target.y - 60 && apex > target.y - PHYSICS.launchClear - 120,
  `the launch tops out just above the shelf (${target ? (target.y - apex).toFixed(0) : '?'} above)`,
);
check(
  Math.abs(engine.player.x - x0) < 40,
  `the launch is straight up (drift ${(engine.player.x - x0).toFixed(0)})`,
);
check(engine.launchLock === false, 'steering is his again once he is up');

if (failures.length) {
  console.error(
    `undercroft: ${failures.length} of ${checks} checks failed\n - ` +
      failures.join('\n - '),
  );
  process.exit(1);
}
console.log(
  `undercroft: ${checks} checks passed (the dark below the 2D routes: floors, pads, the slide back and the launch up)`,
);
