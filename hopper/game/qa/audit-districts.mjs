...
const names = ['world', 'controller', 'jumptuning', 'combattuning', 'district', 'district2', 'district3', 'route', 'scenery', 'trailprops', 'models3d'];
for (const name of names) {
  const raw = fs
    ...
    .replace(/import (\w+) from '(\.\.\/[^']+\.json)';/g, (m, ident, rel) => `const ${ident} = require('${source}${rel}');`);
